...
| `ForwardedHeaders__KnownNetworks` | `10.0.0.0/8` | *(Recommended)* CIDR range(s) of the reverse proxy in front of the API, comma-separated. When set, `X-Forwarded-For` is believed only from these addresses, so a request that bypasses the proxy cannot choose the client IP recorded in login history, on electronic signatures and in rate limits. Unset, it is accepted from any source (a startup warning says so). `ForwardedHeaders__KnownProxies` takes individual addresses the same way. |
| `RateLimiting__Authentication__PermitLimit` | `20` | *(Optional)* Login, password-reset and admin-recovery requests allowed per client IP per `RateLimiting__Authentication__WindowSeconds` (default `60`). `RateLimiting__TokenRefresh__*` (default 60 per 60 s per IP) and `RateLimiting__PasswordChange__*` (default 5 per 900 s per user) work the same way. |
