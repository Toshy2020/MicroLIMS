...
{
    // A step medium's product: its MediaConfiguration's product, else its Material's.
    public static int? ProductOf(TestWorkflowStepMedia stepMedium) =>
        stepMedium.MediaConfiguration?.MediaProductId ?? stepMedium.Material?.MediaProductId;
