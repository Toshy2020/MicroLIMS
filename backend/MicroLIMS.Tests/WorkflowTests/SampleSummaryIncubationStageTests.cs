...
public class SampleSummaryIncubationStageTests
{
    ...
    private static async Task<(int sampleId, TestOrder order, Media media)> SeedTransferOrderAsync(MicroLimsDbContext db)
    {
        ...
        await db.SaveChangesAsync();
        db.TestWorkflowStepMedias.Add(new TestWorkflowStepMedia { TestWorkflowStepId = step.Id, MaterialId = material.Id, TempMin = 30, TempMax = 35, IncubationMinHours = 1, IncubationMaxHours = 1 });
