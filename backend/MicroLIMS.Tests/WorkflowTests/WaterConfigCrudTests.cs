...
public class WaterConfigCrudTests
{
    ...
    {
        await using var db = NewDb();
        var controller = new MicroLIMS.API.Controllers.MasterDataController(db, new MicroLIMS.Application.Services.EquipmentConfigurationService(db), TestServiceFactory.MediaProduct(db), TestServiceFactory.MediaIncubationCondition(db), new MicroLIMS.Application.Services.UserSectionScopeService(db));
